...
const Doctor = () => {
  ...
  const [isChiefDoctorAccess, setIsChiefDoctorAccess] = useState(false);
  ...
  useEffect(() => {
    ...
    if (doctorIdFromUrl) {
      loadDoctorById(parseInt(doctorIdFromUrl), sourceFromUrl === 'chief');
      return;
    }
  ...
  const loadDoctorById = async (doctorId: number, isChief = false) => {
    try {
      ...
      if (response.ok && data.doctors && data.doctors.length > 0) {
        ...
        if (doctor) {
          ...
          setDoctorInfo(doctor);
          setIsAuthenticated(true);
          setIsRegistrarAccess(!isChief);
          setIsChiefDoctorAccess(isChief);
          loadSchedules(doctor.id);
          loadDailySchedules(doctor.id);
          loadAppointments(doctor.id);
          loadCalendar(doctor.id, selectedYear);
      ...
      <Tabs defaultValue="calendar">
        <div className="sticky top-16 z-40 bg-white/95 backdrop-blur-sm border-b border-border shadow-md">
          <div className="container mx-auto px-4 py-3">
            <TabsList className={`grid w-full ${isRegistrarAccess && !isChiefDoctorAccess ? 'grid-cols-2' : 'grid-cols-3'} h-auto p-1 bg-gradient-to-r from-blue-50 to-indigo-50`}>
              <TabsTrigger 
                value="calendar"
              ...
              {(!isRegistrarAccess || isChiefDoctorAccess) && (
                <TabsTrigger 
                  value="appointments"
